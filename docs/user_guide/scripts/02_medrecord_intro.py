...
patients = pd.DataFrame([['Patient 01',72,'M','USA'],
                         ['Patient 02',74,'M','USA'],
                         ['Patient 03',64,'F','GER']],
                         columns=['ID','Age','Sex','Loc'])

# Medications DataFrame (Nodes)
medications = pd.DataFrame([['Med 01','Insulin'],
                            ['Med 02', 'Wararin']],
                            columns=['ID','Name'])

# Patients-Medication Relation (Edges)
pat_med = pd.DataFrame([['Patient 02','Med 01',pd.Timestamp('20200607')],
