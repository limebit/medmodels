...
One important property is *time*. By default, treatment and outcome groups are formed without considering when each event occurred. But when a time attribute is provided, the logic changes: the *Treated* groups _(with or without outcome)_ are now determined based on whether the outcome happened after the treatment. This allows for a more causal interpretation by ensuring that only post-treatment outcomes are considered in the analysis for treated individuals.
