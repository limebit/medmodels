...
class TreatmentEffect:
    ...
    # _matching_warm_init: Optional[Word2Vec] = None[str, Any]
    ...
    def __init__(
        ...
        *,
        ...
        # matching_warm_init: Optional[Word2Vec] = None[str, Any]
    ...
    def _find_groups(
        ...
        treatment_all = self._find_treated_patients(medrecord)
        treatment_all, washout_nodes = self._apply_washout_period(
            medrecord, treatment_all
        )
        treatment_all, treatment_true, outcome_before_treatment_nodes = (
            self._find_outcomes(medrecord, treatment_all)
        )
    ...
    def _find_outcomes(
        self, medrecord: MedRecord, treatment_all: Set[NodeIndex]
        ...
        treatment_all -= outcome_before_treatment_nodes
        ...
        return treatment_all, treatment_true, outcome_before_treatment_nodes
    ...
    def _apply_washout_period(
        ...
        if not self._washout_period_days:
            return treatment_all, washout_nodes
    ...
    def _find_controls(
        ...
        """
        control_all = set(medrecord.group(self._patients_group))

        # Apply the filter to the control group if specified
        if filter_controls_operation:
            control_all = (
                set(medrecord.select_nodes(filter_controls_operation)) & control_all
            )

        control_all = control_all - treatment_all - rejected_nodes
        assert (
            len(control_all) > 0
