...
class TestMedRecord(unittest.TestCase):
    def test_select_nodes_node(self) -> None:
        ...
        assert sorted(["0", "1"]) == sorted(
            medrecord.select_nodes(node().has_neighbor_with(node().index().equal("2")))
        )
        assert sorted(["0"]) == sorted(
    ...
    def test_select_nodes_node_index(self) -> None:
        ...
        # Index greater
        assert sorted(["2", "3"]) == sorted(
            medrecord.select_nodes(node().index().greater("1"))
        )

        # Index less
        assert sorted(["0", "1"]) == sorted(
            medrecord.select_nodes(node().index().less("2"))
        )

        # Index greater or equal
        assert sorted(["1", "2", "3"]) == sorted(
            medrecord.select_nodes(node().index().greater_or_equal("1"))
        )

        # Index less or equal
        assert sorted(["0", "1", "2"]) == sorted(
            medrecord.select_nodes(node().index().less_or_equal("2"))
        )
        ...
        # Index not equal
        assert sorted(["0", "2", "3"]) == sorted(
            medrecord.select_nodes(node().index().not_equal("1"))
        )
        ...
        # Index not in
        assert sorted(["0", "2", "3"]) == sorted(
            medrecord.select_nodes(node().index().not_in(["1"]))
        )
    ...
    def test_select_nodes_node_attribute(self) -> None:
        ...
        assert medrecord.select_nodes(
            node().attribute("lorem").greater_or_equal("ipsum")
        ) == ["0"]
        assert medrecord.select_nodes(node().attribute("lorem") >= "ipsum") == ["0"]

        # Attribute less or equal
        assert medrecord.select_nodes(
            node().attribute("lorem").less_or_equal("ipsum")
        ) == ["0"]
        assert medrecord.select_nodes(node().attribute("lorem") <= "ipsum") == ["0"]
        ...
        # Attribute in
        assert medrecord.select_nodes(node().attribute("lorem").is_in(["ipsum"])) == [
            "0"
        ]
        ...
        assert medrecord.select_nodes(node().attribute("lorem").starts_with("ip")) == [
            "0"
        ]

        # Attribute ends with
        assert medrecord.select_nodes(node().attribute("lorem").ends_with("um")) == [
            "0"
        ]
        ...
        assert medrecord.select_nodes(
            node().attribute("integer").not_equal(node().attribute("integer").sub(10))
        ) == ["0"]
        ...
        assert medrecord.select_nodes(
            node().attribute("integer").not_equal(node().attribute("integer").div(2))
        ) == ["0"]
    ...
    def test_select_edges_edge(self) -> None:
        ...
        assert sorted([1, 2, 3]) == sorted(
            medrecord.select_edges(edge().connected_target("2"))
        )

        # Edge connected to source
        assert sorted([0, 2, 3]) == sorted(
            medrecord.select_edges(edge().connected_source("0"))
        )
    ...
    def test_select_edges_edge_index(self) -> None:
        ...
        # Index greater
        assert sorted([2, 3]) == sorted(
            medrecord.select_edges(edge().index().greater(1))
        )
        ...
        # Index greater or equal
        assert sorted([1, 2, 3]) == sorted(
            medrecord.select_edges(edge().index().greater_or_equal(1))
        )

        # Index less or equal
        assert sorted([0, 1]) == sorted(
            medrecord.select_edges(edge().index().less_or_equal(1))
        )
        ...
        # Index not equal
        assert sorted([0, 2, 3]) == sorted(
            medrecord.select_edges(edge().index().not_equal(1))
        )
        ...
        # Index not in
        assert sorted([0, 2, 3]) == sorted(
            medrecord.select_edges(edge().index().not_in([1]))
        )
    ...
    def test_select_edges_edges_attribute(self) -> None:
        ...
        # Attribute greater or equal
        assert medrecord.select_edges(
            edge().attribute("sed").greater_or_equal("do")
        ) == [0]

        # Attribute less or equal
        assert medrecord.select_edges(edge().attribute("sed").less_or_equal("do")) == [
            0
        ]
        ...
        assert medrecord.select_edges(
            edge().attribute("integer").not_equal(edge().attribute("integer").sub(10))
        ) == [2]
        ...
        assert medrecord.select_edges(
            edge().attribute("integer").not_equal(edge().attribute("integer").div(2))
        ) == [2]
