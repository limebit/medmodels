...
class TestMedRecord(unittest.TestCase):
    ...
    def test_query_nodes(self) -> None:
        ...
        assert sorted(medrecord.query_nodes(query9)) == [0,1,2,3]
    ...
    def test_query_edges(self) -> None:
        ...
        assert [0, 1] == sorted(medrecord.query_edges(query1))
        ...
        def query6(edge: EdgeOperand) -> MultipleAttributesOperand:
            attributes_tree = query5(edge)
            return attributes_tree.max()
        
        assert medrecord.query_edges(query6) == {0: "sed"}

        def query7(edge: EdgeOperand) -> SingleAttributeOperand:
        ...
        assert medrecord.query_edges(query8) == "3"

        def query9(edge: EdgeOperand) -> NodeIndicesOperand:
            return edge.source_node().index()
        
        assert sorted(medrecord.query_edges(query9)) == ["0", "1", "2", "3"]
    ...
    def test_repr(self) -> None:
        ...
        expected = "\n".join([
            ...
            "                                                    max: 2024-04-24 03:38:35 ",
            "-----------------------------------------------------------------------------"
        ]) if sys.gettrace() is None else "<MedRecord: 73 nodes, 160 edges>"
