...
class TestSchema(unittest.TestCase):
    def test_infer(self) -> None:
        ...
        assert len(schema.default.nodes) == 2
        assert len(schema.default.edges) == 1

        medrecord.add_group("test", [0, 1], [0])

        schema = mr.Schema.infer(medrecord)

        assert len(schema.default.nodes) == 0
        assert len(schema.default.edges) == 0
    ...
    def test_from_py_schema(self) -> None:
        ...
        ).default.nodes == {"test": (mr.String(), mr.AttributeType.Unstructured)}

    def test_groups(self) -> None:
        schema = mr.Schema(groups={"test": mr.GroupSchema()}, default=mr.GroupSchema())
    ...
    def test_default(self) -> None:
        schema = mr.Schema(
            groups={}, default=mr.GroupSchema(nodes={"test": mr.String()}, edges={})
        )

        assert schema.default.nodes == {
            "test": (mr.String(), mr.AttributeType.Unstructured)
        }
    ...
    def test_set_node_attribute(self) -> None:
        ...
        assert schema.default.nodes["key1"][0] == mr.Int()

        schema.set_node_attribute("key1", mr.Float(), mr.AttributeType.Continuous)

        assert schema.default.nodes["key1"][0] == mr.Float()
    ...
    def test_set_edge_attribute(self) -> None:
        ...
        assert schema.default.edges["key1"][0] == mr.Bool()

        schema.set_edge_attribute("key1", mr.Float(), mr.AttributeType.Continuous)

        assert schema.default.edges["key1"][0] == mr.Float()
    ...
    def test_update_node_attribute(self) -> None:
        ...
        assert schema.default.nodes["key1"][0] == mr.Union(mr.Int(), mr.Float())
    ...
    def test_update_edge_attribute(self) -> None:
        ...
        assert schema.default.edges["key1"][0] == mr.Union(mr.Bool(), mr.String())
    ...
    def test_remove_node_attribute(self) -> None:
        ...
        assert "key1" in schema.default.nodes

        schema.remove_node_attribute("key1")

        assert "key1" not in schema.default.nodes
    ...
    def test_remove_edge_attribute(self) -> None:
        ...
        assert "key1" in schema.default.edges

        schema.remove_edge_attribute("key1")

        assert "key1" not in schema.default.edges
    ...
    def test_remove_group(self) -> None:
        schema = mr.Schema(
            groups={"group1": mr.GroupSchema()}, default=mr.GroupSchema()
        )
